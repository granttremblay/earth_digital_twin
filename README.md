...
The site is framed around the **SAO + SERC + STRI Innovation Workshop**, **Sept 14–16, 2026**, at the **Smithsonian Astrophysical Observatory** (60 Garden Street, Cambridge, MA), with an informal welcome reception the evening of **Sunday Sept 13**. Hotel block and lodging info are forthcoming. **In-person participation is essential** for the cross-Unit collaboration we're building, so travel for all Smithsonian scientists can be **fully funded** by the OUSSR seed award — but per funding restrictions, **all travel must be booked by July 25, 2026**. Attendees ready to book should email **grant.tremblay@cfa.harvard.edu** immediately so we can coordinate against the seed-award budget. The page includes an **Early Registration** form (`#register`) — name / email / affiliation / comments — wired to **Formspree** (form `xdabeeow`), which forwards submissions to Grant's inbox. Spam is filtered with a hidden `_gotcha` honeypot; reCAPTCHA or Cloudflare Turnstile can be layered on later if needed.
