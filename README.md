...
A dedicated sub-page for confirmed workshop attendees lives at [`website/workshop.html`](website/workshop.html) (served at `https://livingearthtwin.org/workshop.html`). The main page links to it in three places — a **"For Attendees ↗"** nav link, a **"For Workshop Attendees →"** button in the workshop banner, and an *"Already registered?"* line under the registration lede. It reuses the shared design system (`styles.css`) plus a page-specific [`website/workshop.css`](website/workshop.css). Sections: a full-bleed **top hero** on the `workshop_earth_backdrop.png` image with the animated-gradient **"Innovation Workshop"** wordmark (`assets/innovation_workshop.svg`, masked over the pink→purple→blue `--grad-workshop`) offset to the left of the Earth, above a sub-lockup of the Smithsonian logo and the "Living Earth Digital Twin" wordmark (`assets/livingearthdigitaltwin.svg`, inlined and split so "Living Earth" carries the animated gradient while "Digital Twin" stays white); a **quick-links band** directly under the hero with two prominent cards — **Join the Slack** and the **LEDT Cloud Compute Hub** — both pointing at the site's own `slack/` and `hub/` short links so the destinations stay repointable; a three-day **agenda** (an animated-gradient **"Agenda"** wordmark from `assets/agenda.svg`, then Day One · Discover / Day Two · Build / Day Three · Prototype with per-day timelines; every day opens with 09:00 coffee and a 09:30 start; Day One's morning is nine introductory talks split into two duration-based sub-lists either side of a 10:25 coffee break, each talk in the same duration / title / "Speaker · Affiliation" shape, followed by a live platform demo at 11:30; and 14:00 on Day One is a virtual NVIDIA Earth-2 talk); **Travel** logistics over the `workshop_bottom_backdrop.png` image (a full-width **"The space"** venue postcard collage of SAO rooms using `venue-3/4/5`, a two-card venue/airport row, a full-width **"Where to stay"** postcard-collage feature naming the historic **Sheraton Commander** and scattering the three `hotel-1/2/3` images as cream-bordered vintage postcards — each collage is an independent click-to-zoom lightbox gallery — a date band, and the July 25 booking deadline); a **What & Why** section (the "what are we actually doing" wordmark, a short definition of the Heilmeier Catechism, and eight pillboxes answering Heilmeier's questions); **Your Homework** (five steps: "Read the proposal" links to `assets/DigitalTwin_Innovation_SAO_Proposal.pdf`, and a full-width step **embeds an NVIDIA Earth-2 overview video** — a lazy-loaded 16:9 `youtube-nocookie` iframe — with a link to a second short tutorial) (pre-work + the July/August virtual prep calls, plus a **required attendee questionnaire** merged into this section — a red "we really need you to respond" note followed by a Formspree form posting to `https://formspree.io/f/xgogyqla`; the questions are plain `<div class="field">` blocks between `EDIT THE QUESTIONS HERE` comment fences so they're easy to change); a **Poster** download section (a `poster_preview.png` thumbnail linking to `innovation_workshop_poster.pdf`, plus download / preview-in-browser buttons); and a **scrollable, manually-editable participants list** (copy a `<li class="pt-row">` block to add someone — instructions are in an HTML comment). The hero "Innovation Workshop" and the "Agenda", "Day One/Two/Three", "Travel", and "Your Homework" headings are the supplied SVG wordmarks used as CSS `mask`s over the workshop pink→purple→blue gradient (`--grad-workshop`), animated with the shared `gradShift` keyframe. Those wordmark SVGs (`assets/agenda.svg`, `innovation_workshop.svg`, `dayone.svg`, `daytwo.svg`, `daythree.svg`, `travel.svg`, `homework.svg`) had explicit `width`/`height` added so Safari/Firefox size them correctly as masks. Two wordmarks are instead **inlined** so different parts can take different fills: `livingearthdigitaltwin.svg` (hero — "Living Earth" gradient, "Digital Twin" white) and `heilmeier.svg` (rationale — "What are we actually doing?" gradient, "Our Heilmeier Catechism" white).
...
Eleven vanity paths redirect to their destinations, so they can be handed out verbatim:
